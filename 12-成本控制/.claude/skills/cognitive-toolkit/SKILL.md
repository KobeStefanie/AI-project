...
```
/cbt                        # start with check-in ¡ú technique recommendation
...
1. Read `references/thought-record.md` ¡ª thought record protocol (ABC model, cognitive distortion taxonomy, reframe scaffold)
2. Read `references/opposite-action.md` ¡ª opposite action + DEAR MAN + TIPP crisis skills
3. Read `references/pushback-config.md` ¡ª pushback levels, triggers, and mid-session override commands
4. Read `references/health-integration.md` ¡ª HRV/sleep pull, biofeedback interpretation, skip logic

## Session Flow

**Without technique argument** (full flow):
1. Check-in ¡ª mood (0¨C10), brief situation summary
2. Recommend ¡ª match presenting issue to technique based on check-in
3. Load technique ¡ª read the relevant reference file
4. Protocol ¡ª run the full guided exercise
5. Close ¡ª summary, insight, one takeaway
6. Save ¡ª write session to vault output

**With technique argument** (direct jump):
1. Brief mood check (0¨C10, one line)
2. Load technique ¡ª read the relevant reference file
3. Protocol ¡ª run the full guided exercise
4. Close ¡ª summary, insight, one takeaway
5. Save ¡ª write session to vault output
...
- Pushback is Socratic, not confrontational ¡ª "What evidence supports that?" not "That's wrong"
...
- Never gate a session on health data ¡ª it's context, not gatekeeper
...
- NOT a diagnostic tool ¡ª never assess, label, or diagnose
- NOT a therapist replacement ¡ª this is a practice tool for between sessions
