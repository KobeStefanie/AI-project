...
A specific, first-person thought is captured 〞 not an emotion, not a behavior, not an edited version. It should be possible to rate it for belief intensity (0每100%) if needed.
...
**Primary prompt:**
> "What emotion did that thought bring up? And how intense was it, on a 0每10 scale where 10 is the strongest you've ever felt it?"
...
At least one emotion is named with an intensity rating (0每10). Record as baseline.
...
At least 1每3 pieces of evidence for the thought are listed, however weak. The step doesn't require the evidence to be strong 〞 it requires the thought to be taken seriously.
...
### Socratic questions (use 1每2, not all 〞 avoid interrogation feel)
...
At least 1 piece of counter-evidence is captured, or user has been given 2每3 genuine opportunities and unable to produce any (logged as skip).
...
- "Where would you rate your belief in that, 0每100%?"
...
**Primary prompt:**
> "How are you feeling now? Same 0每10 scale as before."
...
Belief intensity (pre): [0每100% if captured]
...
Delta: [+/每 points] [e.g., "每3 (significant shift)" / "no change" / "+1 (temporary intensification)"]

**Genetic Context** *(include only if relevant genomic data is available)*
[e.g., "BDNF Val/Val: cognitive flexibility typically requires more repetitions to consolidate. Consider revisiting this thought record in 24每48 hours."]
