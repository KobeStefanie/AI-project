...
¡ú extract HRV value
¡ú store as hrv_pre in session frontmatter
```
...
¡ú extract HRV value
¡ú store as hrv_post
¡ú calculate hrv_delta = hrv_post - hrv_pre
¡ú log in session notes
```
...
| Metric | Range | Notes |
|--------|-------|-------|
| Readiness score | 0¨C100 | Composite recovery indicator |
| HRV | continuous | Higher resolution than Apple Health |
| Skin temperature delta | ¡ãC delta | Deviation from personal baseline |
| Sleep staging | light/deep/REM/awake | Nightly breakdown |

### Session Context Rules

- **Low readiness (< 60)** ¡ú suggest gentler session; prioritise grounding and somatic work over deep cognitive challenge
- **Medium readiness (60¨C80)** ¡ú standard session, no adjustment needed
- **High readiness (> 80)** ¡ú good time for deeper work; cognitive restructuring, schema exploration, intensive thought records
...
1. Check if `~/Brains/genome/` exists ¡ª skip silently if not
...
**NEVER invent or estimate health values.** If a value is unavailable, omit it entirely ¡ª do not substitute averages, guesses, or placeholders.
